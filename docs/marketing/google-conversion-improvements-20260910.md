...
The owner also reported a product purchase bar covering consent actions in Facebook's in-app browser. The combined frontend patch uses a body portal, bounded scrollable purpose area, a separate non-shrinking action footer, and three 44px choice targets. It changes layout only; all purposes, copy, setters, and save/refusal semantics are preserved. G152's older two-choice target expectation was updated to require all three named choices and five total large controls.

Local evidence: G189 26 assertions plus 4 rendered cases; G196 15/15; full lint/typecheck, diagnostic baseline, tracked and new-file secret scans, and diff-check pass. Isolated WebKit mobile/tablet/desktop and three-choice consent checks pass with all external traffic blocked and public API responses mocked, including core, extended/route-review minimums, lookup failures, waitlist, and legacy-cache cases. Twelve additional integrated product-page cases pass across six viewport sizes in both light and dark themes: five-point footer hit-testing under root stacking isolation, all three 44px choices, save/refusal persistence, and Add to cart/cart navigation after refusal. These tests do not substitute for live post-deploy or physical Facebook-browser verification. Final critical-suite results are recorded with the release evidence.

This activation is site-only. The ZIP card also appears on Home and in Cart's optional prompt, so installed native copies require a separate Appflow/native release to receive that shared UI correction. New Trio lifestyle photos remain separate owner-review assets and are not published by this source change.
