...
for (const product of consumables) {
  ...
  if (approved) {
    assert.equal(gallery[0].thumbnail, approved.card, `${product.title} must use its matching square thumbnail`);
    assert.equal(gallery.some(image => image.src === product.image_url), false, `${product.title} must not show its retired catalog original`);
  } else {
    assert.equal(gallery[0].src, product.image_url, `${product.title} must preserve its existing catalog primary`);
  }
  ...
  assert.equal(structuredData.image.includes(product.image_url), !approved, `${product.title} structured data must reflect visible catalog-original policy`);
...
const oasisWithExistingSecondary = {
  ...
  secondary_images: [
    ...
    PUBLIC_PRODUCT_FALLBACKS.find(product => product.title === 'OASIS').image_url,
    '/images/oasis-detail.jpg',
    'https://example.com/oasis-real-secondary.jpg',
  ],
};
const oasisGallery = buildProductGallery(oasisWithExistingSecondary);
assert.equal(oasisGallery.length, 6, 'OASIS should retain the approved primary and unique real secondary photos before authentic scenes, without the retired original');
assert.equal(oasisGallery.some(image => image.src === oasisWithExistingSecondary.image_url), false);
...
console.log(JSON.stringify({
  ...
  retired_target_catalog_originals_hidden: true,
  non_target_catalog_primaries_preserved: true,
