...
const saveEntry = read('base44/functions/getAdminOperationsDashboardSummary/handlers/saveAdminComplianceRecord/entry.ts');
const complianceSummaryEntry = read('base44/functions/getAdminOperationsDashboardSummary/handlers/getAdminComplianceOpsSummary/entry.ts');
...
const productionQueueEntry = read('base44/functions/getAdminOperationsDashboardSummary/handlers/getAdminProductionQueueSummary/entry.ts');
const previewStartEntry = read('base44/functions/getAdminOperationsDashboardSummary/handlers/previewAdminProductionBatchStart/entry.ts');
const liveStartEntry = read('base44/functions/getAdminOperationsDashboardSummary/handlers/startAdminProductionBatch/entry.ts');
...
assert('Compliance summary makes native records the top-level operational truth.', complianceSummaryEntry.includes("source: 'customer_app_native_compliance_authoritative'") && complianceSummaryEntry.includes('summary: native.summary') && complianceSummaryEntry.includes('records: native.records'), {});
assert('Compliance summary does not fetch Hub during the default daily read.', complianceSummaryEntry.includes('if (!includeHubHistoricalContext)') && complianceSummaryEntry.includes('return Response.json(nativeResponse)'), {});
assert('Compliance summary requires an explicit historical-context request for Hub.', complianceSummaryEntry.includes('body.include_hub_historical_context === true') && complianceSummaryEntry.includes('hub_historical_context_requested: includeHubHistoricalContext'), {});
assert('Hub compliance history cannot change native operational totals or records.', complianceSummaryEntry.includes('hub_historical_context_summary: safeObjectNumberMap(hub.summary)') && complianceSummaryEntry.includes('hub_historical_context_excluded_from_operational_totals: true'), {});
