...
const ACTUAL_UNITS = Object.fromEntries(BATCH_IDS.map(id => [id, 1]));
...
function loadCommandHarness(env = {}) {
  ...
  source += `\nglobalThis.__exports = { gateFailure, exactTargetBlockers, parseActualUnitsMap, validateFreshPreview, preflightTargetBatches, buildCompletePatch, validateCompletePatch, updateProductionBatches, requireAdmin };\n`;
...
function loadPreviewHarness() {
  ...
  let source = fs.readFileSync(filePath, 'utf8');
  source = source.replace(/^import .*$/gm, '');
  source += `\nglobalThis.__exports = { planComplete, buildBatchLifecycleRow, buildOrderLifecyclePreview, getLookup };\n`;
...
function makeLifecyclePreview(overrides = {}) {
  ...
  return {
    ...
    start_preview: { action: 'start', preview_only: true, ready_count: 0, blocked_count: 6, already_started_count: 6, ready_batch_ids: [], no_writes_now: true },
    complete_preview: { action: 'complete', preview_only: true, ready_count: 6, blocked_count: 0, ready_batch_ids: BATCH_IDS, no_writes_now: true },
...
function liveBody(overrides = {}) {
  return {
    ...
    production_date: '2026-06-05',
    batch_ids: BATCH_IDS,
    batch_actual_units: ACTUAL_UNITS,
    customer_app_order_id: '6a219a3f4adcda5856c3d579',
    native_shopify_order_id: '6a22ffda400eb806eb3ca945',
    native_fulfillment_task_id: '6a22ffdaf675ea79e30575aa',
    expected_status: 'in_production',
    request_id: 'g31r_complete_test',
    ...overrides,
  };
}

const previewHarness = loadPreviewHarness();
const previewBatch = makeBatches()[0];
const pendingCompletePlan = previewHarness.exports.planComplete({ batch: previewBatch, actorEmail: 'owner@example.test', requestId: 'preview_pending', now: '2026-06-08T04:00:00.000Z', completionInput: {} });
assert.ok(pendingCompletePlan.blockers.includes('missing_actual_units'));
const readyCompletePlan = previewHarness.exports.planComplete({ batch: previewBatch, actorEmail: 'owner@example.test', requestId: 'preview_ready', now: '2026-06-08T04:00:00.000Z', completionInput: { actual_units: 0 } });
assert.equal(readyCompletePlan.blockers.length, 0);
assert.equal(readyCompletePlan.proposed_patch.actual_units, 0);
assert.equal(readyCompletePlan.proposed_patch.ingredients_used, undefined);
assert.equal(readyCompletePlan.proposed_patch.pH_result, undefined);
assert.equal(readyCompletePlan.proposed_patch.compliance_log_id, undefined);
...
assert.ok(fns.exactTargetBlockers({ ...lookup, expectedStatus: 'planned' }).includes('expected_status_must_be_in_production'));
assert.ok(fns.exactTargetBlockers({ ...lookup, batchIds: BATCH_IDS.slice(0, 5) }).includes('target_batch_ids_mismatch'));
assert.equal(fns.parseActualUnitsMap({ batch_actual_units: ACTUAL_UNITS }).blockers.length, 0);
assert.ok(fns.parseActualUnitsMap({ batch_actual_units: { [BATCH_IDS[0]]: 1 } }).blockers.includes('exact_batch_actual_units_required'));
assert.ok(fns.parseActualUnitsMap({ batch_actual_units: { ...ACTUAL_UNITS, [BATCH_IDS[0]]: -1 } }).blockers.some(item => item.startsWith('invalid_actual_units')));
...
env.NATIVE_PRODUCTION_BATCH_COMPLETE_ORDER_ALLOWLIST = 'NV-MPZNKGNT';
...
let response = await handler(req(unauthStore.base44, liveBody({ request_id: 'g31r_unauth' })));
...
response = await handler(req(nonAdminStore.base44, liveBody({ request_id: 'g31r_forbidden' })));
...
response = await disabledHarness.handler(req(disabledStore.base44, liveBody({ request_id: 'g31r_disabled' })));
...
response = await handler(req(makeStore().base44, liveBody({ confirmation: 'wrong_phrase', request_id: 'g31r_bad_confirmation' })));
assert.equal(response.status, 400);
assert.equal((await json(response)).error_code, 'confirmation_required');

response = await handler(req(makeStore().base44, liveBody({ batch_actual_units: undefined, request_id: 'g31r_missing_units' })));
assert.equal(response.status, 409);
assert.equal((await json(response)).error_code, 'exact_complete_target_required');

response = await handler(req(makeStore().base44, liveBody({ batch_actual_units: { ...ACTUAL_UNITS, [BATCH_IDS[0]]: -1 }, request_id: 'g31r_invalid_units' })));
assert.equal(response.status, 409);
assert.equal((await json(response)).error_code, 'exact_complete_target_required');

response = await handler(req(makeStore().base44, liveBody({ batch_actual_units: Object.fromEntries(BATCH_IDS.slice(0, 5).map(id => [id, 1])), request_id: 'g31r_missing_one_units' })));
assert.equal(response.status, 409);
assert.equal((await json(response)).error_code, 'exact_complete_target_required');

const notReady = fns.validateFreshPreview(makeLifecyclePreview({ complete_preview: { ready_count: 5 }, batch_count: 6 }), lookup);
...
response = await handler(req(liveStore.base44, liveBody({ request_id: 'g31r_complete_live' })));
...
assert.equal(liveStore.store.productionBatches.every(batch => batch.actual_units === 1), true);
assert.equal(liveStore.store.productionBatches.every(batch => Boolean(batch.actual_end_time)), true);
assert.equal(liveStore.store.productionBatches.every(batch => batch.completed_by === 'owner@example.test'), true);
...
const alreadyCompletedOverrides = Object.fromEntries(BATCH_IDS.map(id => [id, { status: 'completed_pending_verification', actual_units: 1, actual_end_time: '2026-06-08T05:00:00.000Z', completed_by: 'owner@example.test' }]));
const alreadyCompletedStore = makeStore({ productionBatches: makeBatches(alreadyCompletedOverrides), preview: validPreview });
response = await handler(req(alreadyCompletedStore.base44, liveBody({ request_id: 'g31r_already_completed' })));
body = await json(response);
assert.equal(response.status, 200);
assert.equal(body.skipped, true);
assert.equal(body.reason, 'exact_native_batches_already_completed_pending_verification');
assert.equal(alreadyCompletedStore.store.productionBatches.every(batch => !Array.isArray(batch.audit_trail)), true);
...
response = await handler(req(plannedStore.base44, liveBody({ request_id: 'g31r_planned_blocked' })));
body = await json(response);
assert.equal(response.status, 409);
...
const partialStore = makeStore({ productionBatches: makeBatches({ [BATCH_IDS[0]]: { status: 'completed_pending_verification', actual_units: 1, actual_end_time: '2026-06-08T05:00:00.000Z', completed_by: 'owner@example.test' } }), preview: validPreview });
response = await handler(req(partialStore.base44, liveBody({ request_id: 'g31r_partial' })));
body = await json(response);
assert.equal(response.status, 409);
...
response = await handler(req(lockedStore.base44, liveBody({ request_id: 'g31r_locked' })));
body = await json(response);
assert.equal(response.status, 409);
assert.equal(body.error_code, 'lifecycle_conflict');
...
const terminalStore = makeStore({ productionBatches: makeBatches({ [BATCH_IDS[0]]: { status: 'verified_logged', verified_at: '2026-06-08T06:00:00.000Z' } }), preview: validPreview });
response = await handler(req(terminalStore.base44, liveBody({ request_id: 'g31r_terminal' })));
body = await json(response);
assert.equal(response.status, 409);
assert.equal(body.error_code, 'lifecycle_conflict');
assert.equal(terminalStore.store.commandLogs.length, 0);

const mismatchStore = makeStore({ preview: makeLifecyclePreview({ complete_preview: { ready_count: 6 }, batch_lifecycle_rows: validPreview.batch_lifecycle_rows.slice(0, 5) }) });
response = await handler(req(mismatchStore.base44, liveBody({ request_id: 'g31r_preview_mismatch' })));
body = await json(response);
assert.equal(response.status, 409);
...
response = await handler(req(forbiddenStore.base44, liveBody({ request_id: 'g31r_forbidden_payload', ingredients_used: [] })));
body = await json(response);
assert.equal(response.status, 400);
...
console.log('G31R native ProductionBatch complete command tests passed');
