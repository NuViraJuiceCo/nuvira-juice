...
const catalogPricing = read('base44/functions/createPaymentIntent/rewardCheckout.js');
...
assert.match(catalogPricing, /entities\.Product\.filter\(\s*\{ is_available: true \},\s*'sort_order',\s*250/);
assert.match(catalogPricing, /const byId = catalogIndex\(products\)/);
assert.match(catalogPricing, /products\.length >= 250/);
assert.match(catalogPricing, /byId\.has\(product\.id\)/);
