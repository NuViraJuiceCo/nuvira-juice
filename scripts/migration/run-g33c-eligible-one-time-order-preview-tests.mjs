...
function loadHarness({ env = {} } = {}) {
  ...
  source += `\nglobalThis.__exports = { G33C_READ_ONLY_SAFETY, g33cLookup, g33cUnsupportedBodyKey, buildG33CPreview, isG33CPreviewRequest, G33C_PREVIEW_MODE };\n`;
...
function makeComplianceLog(overrides = {}) {
  return { id: 'compliance_good', batch_id: 'G33C-BATCH-GOOD', source_production_batch_id: 'batch_good', locked: true, ...overrides };
}

function makeStore({
  user = { role: 'admin', email: 'owner@example.test' },
  orders = [makeOrder()], nativeOrders = [makeNativeOrder()], tasks = [makeTask()], orderSyncLogs = [], reviewRows = [], commandLogs = [], parityLogs = [], batches = [], complianceLogs = [],
} = {}) {
  const store = { orders, nativeOrders, tasks, orderSyncLogs, reviewRows, commandLogs, parityLogs, batches, complianceLogs, writes: [] };
  const rowsFor = name => ({ Order: store.orders, ShopifyOrder: store.nativeOrders, FulfillmentTask: store.tasks, OrderSyncLog: store.orderSyncLogs, OrderReviewQueue: store.reviewRows, CommandLog: store.commandLogs, SafeSyncParityLog: store.parityLogs, ProductionBatch: store.batches, BatchComplianceLog: store.complianceLogs }[name] || []);
  ...
  return {
    ...
    base44: {
      ...
      asServiceRole: { entities: { Order: api('Order'), ShopifyOrder: api('ShopifyOrder'), FulfillmentTask: api('FulfillmentTask'), OrderSyncLog: api('OrderSyncLog'), OrderReviewQueue: api('OrderReviewQueue'), CommandLog: api('CommandLog'), SafeSyncParityLog: api('SafeSyncParityLog'), ProductionBatch: api('ProductionBatch'), BatchComplianceLog: api('BatchComplianceLog') } },
    },
  };
}
...
async function previewFor(scenario, body) { return fns.buildG33CPreview(scenario.base44, body); }

const { exports: fns, handler, source } = loadHarness({ env: { NATIVE_SAFE_SYNC_PREVIEW_SECRET: 'preview-secret' } });

assert.equal(fns.G33C_PREVIEW_MODE, 'ELIGIBLE_ONE_TIME_ORDER_NATIVE_WORKFLOW');
assert.equal(fns.G33C_READ_ONLY_SAFETY.dry_run_only, true);
assert.equal(fns.G33C_READ_ONLY_SAFETY.writes_performed, false);
assert.equal(fns.G33C_READ_ONLY_SAFETY.provider_calls_performed, false);
assert.equal(fns.G33C_READ_ONLY_SAFETY.stripe_calls_performed, false);
assert.equal(fns.G33C_READ_ONLY_SAFETY.shopify_api_calls_performed, false);
assert.equal(fns.g33cUnsupportedBodyKey({ mode: 'EXACT_ORDER_PREVIEW', order_number: 'NV-GOOD', send_notification: true }), 'send_notification');
assert.equal(fns.g33cLookup({ order_number: 'NV-GOOD' }).mode, 'EXACT_ORDER_PREVIEW');
assert.equal(fns.g33cLookup({}).mode, 'RECENT_CANDIDATE_SCAN');
assert.equal(fns.isG33CPreviewRequest({ preview_mode: 'ELIGIBLE_ONE_TIME_ORDER_NATIVE_WORKFLOW', mode: 'dry_run' }), true);

let scenario = makeStore();
let preview = await previewFor(scenario, { order_number: 'NV-G33C-GOOD' });
assert.equal(preview.success, true);
assert.equal(preview.dry_run, true);
assert.equal(preview.writes_performed, false);
assert.equal(preview.function_name, 'previewNativeOrderCutoverReadiness');
assert.equal(preview.requested_function_alias, 'previewEligibleOneTimeOrderNativeWorkflow');
...
preview = await previewFor(scenario, { order_number: 'NV-G33C-GOOD' });
...
scenario = makeStore({ orders: [makeOrder({ status: 'delivered' })], nativeOrders: [makeNativeOrder({ production_status: 'bottled', fulfillment_status: 'fulfilled' })], tasks: [makeTask({ status: 'delivered', delivery_status: 'delivered' })] });
preview = await previewFor(scenario, { order_number: 'NV-G33C-GOOD' });
assert.equal(preview.candidate_rows[0].eligible_for_second_controlled_pilot, false);
...
scenario = makeStore({ orders: [makeOrder({ id: 'order_good', order_number: 'NV-G33C-GOOD' }), makeOrder({ id: 'order_pending', order_number: 'NV-G33C-PENDING', payment_status: 'pending', payment_captured: false })], nativeOrders: [makeNativeOrder()], tasks: [makeTask()] });
preview = await previewFor(scenario, { mode: 'RECENT_CANDIDATE_SCAN', max_recent_candidates: 2 });
...
preview = await previewFor(scenario, { customer_app_order_id: 'order_good' });
...
let res = await handler(req(scenario.base44, { mode: 'EXACT_ORDER_PREVIEW', preview_mode: 'ELIGIBLE_ONE_TIME_ORDER_NATIVE_WORKFLOW', order_number: 'NV-G33C-GOOD' }));
...
scenario = makeStore();
res = await handler(req(scenario.base44, { mode: 'EXACT_ORDER_PREVIEW', preview_mode: 'ELIGIBLE_ONE_TIME_ORDER_NATIVE_WORKFLOW', order_number: 'NV-G33C-GOOD' }));
...
const forbiddenSnippets = ['sendOrderStatusNotification', 'pushNotification', 'sendSMS', 'sendEmail', 'stripe.', 'Shopify(', 'runSyncRepair', 'runReplay', 'deductInventory', 'PurchaseOrder.create', 'disableHub'];
