 ...
 * Creates a Stripe HOSTED checkout session (redirect-based) for subscriptions.
 * This is the most reliable path across all devices and browsers.
 * The webhook handles checkout.session.completed exactly as before.
 */
...
function resolveSubscriptionFirstFulfillment(orderTimestamp, options = {}) {
  ...
  else if (dow === 2) { daysToNextProduction = parseInt(pm.hour) < cutoffHour ? 0 : 3; reason = parseInt(pm.hour) < cutoffHour ? 'Tuesday before cutoff' : 'Tuesday after cutoff → Friday'; }
...
Deno.serve(async (req) => {
  try {
    ...
    try {
      ...
      console.warn(`[SubHosted] Failed to fetch UserProfile: ${err.message}`);
    }
    ...
    console.log(`[SubHosted] Fulfillment: production=${fulfillmentCalc.production_date}, first_delivery=${fulfillmentCalc.first_delivery_date}`);
    ...
    const itemsSummaryStr = products.length > 0 ? products.map(p => `${p.quantity}x ${p.product_name}`).join(', ') : subscriptionPlan.name;
    ...
    console.log(`[SubHosted] Created PendingSubscriptionCheckout: ${pendingCheckout.id}`);

    const metadata = {
      base44_app_id: Deno.env.get('BASE44_APP_ID'),
      source_app: 'customer_app',
      checkout_version: '3.0_hosted',
    ...
    const origin = clientOrigin || req.headers.get('origin') || 'https://www.nuvirajuice.com';

    // Create HOSTED Stripe Checkout Session (redirect-based — works on all devices)
    const session = await stripe.checkout.sessions.create({
      mode: 'subscription',
      customer: stripeCustomer.id,
      line_items: [{ price: subscriptionPlan.stripe_price_id, quantity: 1 }],
      subscription_data: { metadata },
      metadata,
      success_url: `${origin}/account/subscriptions?session_id={CHECKOUT_SESSION_ID}&subscribed=true`,
    ...
    }).catch(e => console.warn(`[SubHosted] Failed to update pending checkout: ${e.message}`));

    console.log(`[SubHosted] Created hosted session ${session.id} for ${customer_email}, plan: ${subscriptionPlan.name}`);
    ...
    console.error('[SubHosted] Error:', error.message);
