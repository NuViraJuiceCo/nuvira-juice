...
const INTERNAL_TEST_CUSTOMER_EMAIL = 'info@nuvirajuice.com';
const INTERNAL_TEST_ORDER_PREFIX = 'G81-TEST-';
...
const ALLOWED_BODY_KEYS = new Set([
  ...
  'notify_customer',
  'allow_internal_test_customer_side_effects',
]);
...
function customerOrderKeysFromBatch(batch) {
  const ids = new Set();
  const numbers = new Set();
  for (const value of Array.isArray(batch?.related_orders) ? batch.related_orders : []) {
    const id = sanitizeId(value);
    if (id) ids.add(id);
  }
  for (const source of Array.isArray(batch?.order_sources) ? batch.order_sources : []) {
    const row = safeObject(source);
    for (const value of [row.customer_app_order_id, row.base44_order_id, row.order_id]) {
      const id = sanitizeId(value);
      if (id) ids.add(id);
    }
    const number = normalizedOrderNumber(row.order_number);
    if (number) numbers.add(number);
  }
  return { ids: [...ids].slice(0, 25), numbers: [...numbers].slice(0, 25) };
}

async function findCustomerOrdersForBatch(base44, batch) {
  const { ids, numbers } = customerOrderKeysFromBatch(batch);
  const matches = new Map();
  for (const id of ids) {
    const order = await base44.asServiceRole.entities.Order.get(id).catch(() => null);
    if (order?.id) matches.set(order.id, order);
  }
  for (const number of numbers) {
    const rows = await base44.asServiceRole.entities.Order.filter({ order_number: number }, '-created_date', 2).catch(() => []);
    if (Array.isArray(rows) && rows.length === 1 && rows[0]?.id) matches.set(rows[0].id, rows[0]);
  }
  return [...matches.values()];
}

function safeInternalTestOrder(order) {
  const total = Number(order?.total);
  return Boolean(
    order?.id && order?.is_test_order === true &&
    normalizedOrderNumber(order?.order_number).startsWith(INTERNAL_TEST_ORDER_PREFIX) &&
    normalizeLower(order?.customer_email) === INTERNAL_TEST_CUSTOMER_EMAIL &&
    normalizeLower(order?.payment_status) === 'paid' && order?.payment_captured === true &&
    Number.isFinite(total) && total === 0 &&
    !normalizeText(order?.stripe_payment_intent_id) && !normalizeText(order?.stripe_checkout_session_id)
  );
}

async function validateInternalTestOrderProjection({ base44, batch, action, body }) {
  const requested = boolFlag(body.update_customer_order_status) || boolFlag(body.notify_customer);
  if (!isInternalTestBatch(batch) || !requested) return { ok: true, orders: null };
  if (action !== 'start') return { ok: false, error_code: 'test_batch_customer_projection_action_not_allowed' };
  if (!boolFlag(body.allow_internal_test_customer_side_effects)) {
    return { ok: false, error_code: 'test_batch_customer_side_effects_forbidden' };
  }
  const orders = await findCustomerOrdersForBatch(base44, batch);
  if (orders.length === 0 || !orders.every(safeInternalTestOrder)) {
    return { ok: false, error_code: 'internal_test_customer_projection_isolation_failed' };
  }
  return { ok: true, orders };
}

function terminalCustomerOrder(order) {
  return ['delivered', 'picked_up', 'cancelled', 'canceled', 'refunded', 'failed'].includes(normalizeLower(order?.status)) ||
    ['refunded', 'failed'].includes(normalizeLower(order?.payment_status)) || order?.do_not_recover === true;
}

async function projectLinkedCustomerOrdersInProduction({ base44, batch, requestId, now, preloadedOrders }) {
  const orders = Array.isArray(preloadedOrders) ? preloadedOrders : await findCustomerOrdersForBatch(base44, batch);
  const result = { attempted: true, matched_count: orders.length, updated_count: 0, skipped_count: 0, notification_queued_count: 0, skips: [] };
  for (const order of orders) {
    if (terminalCustomerOrder(order)) {
      result.skipped_count += 1;
      result.skips.push('terminal_or_refunded_order');
      continue;
    }
    if (normalizeLower(order?.payment_status) !== 'paid' || order?.payment_captured !== true) {
      result.skipped_count += 1;
      result.skips.push('order_not_paid');
      continue;
    }
    if (normalizeLower(order?.status) === 'in_production') {
      result.skipped_count += 1;
      result.skips.push('already_in_production');
      continue;
    }
    const history = Array.isArray(order?.status_history) ? order.status_history.slice(-100) : [];
    await base44.asServiceRole.entities.Order.update(order.id, {
      ...
      status_history: [...history, {
        status: 'in_production',
        timestamp: now,
        message: 'Production started for this order.',
        request_id: sanitizeId(requestId) || null,
      }],
    });
    result.updated_count += 1;
    result.notification_queued_count += 1;
  }
  result.skips = [...new Set(result.skips)];
  return result;
}
...
export default async function handler(req: Request) {
  try {
    ...
    const internalTestProjectionGate = await validateInternalTestOrderProjection({ base44, batch, action, body });
    if (!internalTestProjectionGate.ok) {
      return Response.json({
        success: false,
        skipped: true,
        error_code: internalTestProjectionGate.error_code,
        native_writer_enabled: true,
        writes_performed: false,
      ...
      if (action === 'start' && boolFlag(body.update_customer_order_status) && customerProjectionSuppressed(batch)) {
        ...
        try {
          customerProjection = await projectLinkedCustomerOrdersInProduction({
            ...
            requestId,
            now,
            preloadedOrders: internalTestProjectionGate.orders,
          });
        } catch (error) {
          ...
          customerProjection = {
            ...
            skipped_count: 0,
            notification_queued_count: 0,
            skips: ['customer_order_projection_failed'],
