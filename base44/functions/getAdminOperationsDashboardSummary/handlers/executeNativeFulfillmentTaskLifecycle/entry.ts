...
async function validateInternalTestCustomerSideEffects({ base44, task, action, body }) {
  const sideEffectsRequested = boolFlag(body.update_customer_order_status) || boolFlag(body.notify_customer);
  if (task?.is_test_task !== true || !sideEffectsRequested) return { ok: true, order: null };
  if (!boolFlag(body.allow_internal_test_customer_side_effects)) {
    return { ok: false, error_code: 'test_task_customer_side_effects_forbidden' };
  }
  if (!['out_for_delivery', 'delivered_operational'].includes(action)) {
    return { ok: false, error_code: 'test_task_customer_side_effect_action_not_allowed' };
  }

  const order = await findCustomerOrderForTask(base44, task);
  const orderNumber = normalizedOrderNumber(order?.order_number);
  const customerEmail = normalizeLower(order?.customer_email);
  const total = Number(order?.total);
  const safelyIsolated = Boolean(
    order?.id &&
    order?.is_test_order === true &&
    orderNumber.startsWith(INTERNAL_TEST_ORDER_PREFIX) &&
    customerEmail === INTERNAL_TEST_CUSTOMER_EMAIL &&
    normalizeLower(order?.payment_status) === 'paid' &&
    order?.payment_captured === true &&
    Number.isFinite(total) && total === 0 &&
    !normalizeText(order?.stripe_payment_intent_id) &&
    !normalizeText(order?.stripe_checkout_session_id)
  );
  if (!safelyIsolated) {
    return { ok: false, error_code: 'internal_test_customer_side_effect_isolation_failed' };
  }
...
function envGateFailure({ action, task, requestedKey, actorEmail }) {
  ...
  const isTestRequest = isInternalTestTask(task);
  ...
  if (isTestRequest) {
    ...
    if (allowedTasks.size === 0) return 'test_task_allowlist_required';
    const keys = taskAllowlistKeys(task, requestedKey);
    if (!keys.some((key) => allowedTasks.has(key))) return 'test_task_not_allowlisted';
  }
...
async function createCommandLog({ base44, task, action, status, idempotencyKey, requestId, user, result, errorCode, errorMessage }) {
  ...
  return base44.asServiceRole.entities.CommandLog.create({
    ...
    payload: {
      action,
      operational_task_authorized: !isInternalTestTask(task),
      test_task_allowlist_enforced: isInternalTestTask(task),
...
export default async function handler(req: Request) {
  try {
    ...
    const internalTestSideEffectGate = await validateInternalTestCustomerSideEffects({
      base44,
      task,
      action,
      body,
    });
    if (!internalTestSideEffectGate.ok) {
      return Response.json({
        success: false,
        skipped: true,
        error_code: internalTestSideEffectGate.error_code,
        native_writer_enabled: true,
        writes_performed: false,
    ...
    if (notifyCustomer) {
      customerNotification = customerProjection.updated === true
        ? {
            attempted: true,
            sent: false,
            skipped: false,
            queued: true,
            reason: 'order_status_entity_automation_triggered',
          }
        : {
            attempted: false,
            sent: false,
            skipped: true,
            reason: customerProjection.reason || 'customer_order_projection_missing',
          };
    }
