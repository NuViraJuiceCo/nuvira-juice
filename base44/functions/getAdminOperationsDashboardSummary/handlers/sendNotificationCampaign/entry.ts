...
const VALID_AUDIENCES = new Set(['all_customers', 'active_subscribers', 'one_time_customers', 'lapsed_customers', 'test_only']);
const VALID_NOTIFICATION_TYPES = new Set(['order_update', 'promotion', 'new_drop', 'general']);
...
function requestId(value: unknown): string {
  ...
  if (!normalized || normalized.length > 180 || !/^[A-Za-z0-9._:-]+$/.test(normalized)) return '';
  return normalized;
}
...
function safeDeepLink(value: unknown): string | null {
  ...
  if (!normalized.startsWith('/') || normalized.startsWith('//') || normalized.length > 300) return null;
  return normalized;
}

async function updateCommand(base44: any, commandLogId: string | null, patch: Record<string, any>) {
  if (!commandLogId) return;
  await base44.asServiceRole.entities.CommandLog.update(commandLogId, patch).catch((error: unknown) => {
    console.error('[sendNotificationCampaign] Unable to finalize command log:', errorMessage(error));
  });
}
...
export default async function handler(req: Request) {
  try {
    ...
    const action = normalizeSingleLine(body.action || 'send_existing').toLowerCase();
    let {
      campaign_id,
      confirm = false,
    ...
    let commandLogId: string | null = null;

    if (!['send_existing', 'create_and_send'].includes(action)) {
      return Response.json({ error: 'unsupported_action' }, { status: 400 });
    }

    if (action === 'create_and_send') {
      const title = normalizeSingleLine(body.title).slice(0, 160);
      const message = normalizeSingleLine(body.message).slice(0, 1000);
      const audience = normalizeSingleLine(body.audience).toLowerCase();
      const notificationType = normalizeSingleLine(body.notification_type || 'promotion').toLowerCase();
      const deepLink = safeDeepLink(body.deep_link);
      const clientRequestId = requestId(body.request_id);
      if (!title || !message) return Response.json({ error: 'campaign_title_and_message_required' }, { status: 400 });
      if (!VALID_AUDIENCES.has(audience)) return Response.json({ error: 'unsupported_campaign_audience' }, { status: 400 });
      if (!VALID_NOTIFICATION_TYPES.has(notificationType)) return Response.json({ error: 'unsupported_notification_type' }, { status: 400 });
      if (normalizeSingleLine(body.deep_link) && !deepLink) return Response.json({ error: 'deep_link_must_be_a_local_app_path' }, { status: 400 });
      if (!clientRequestId) return Response.json({ error: 'valid_request_id_required' }, { status: 400 });
      if (!confirm) return Response.json({ error: 'Must pass confirm=true to execute campaign send. This is a safety gate.' }, { status: 400 });
      if (audience !== 'test_only') {
        const expectedConfirmation = broadSendConfirmationPhrase(audience);
        if (normalizeSingleLine(broad_send_confirmation) !== expectedConfirmation) {
          return Response.json({ error: 'broad_campaign_confirmation_required', required_confirmation: expectedConfirmation }, { status: 409 });
        }
        if (!positiveInteger(max_recipient_ack)) {
          return Response.json({ error: 'broad_campaign_recipient_ack_required' }, { status: 409 });
        }
      }

      const idempotencyKey = `notification_campaign_create_and_send:${clientRequestId}`;
      const existingCommands = await base44.asServiceRole.entities.CommandLog.filter({ idempotency_key: idempotencyKey }, '-created_date', 5).catch(() => []);
      const existingCommand = Array.isArray(existingCommands) ? existingCommands[0] : null;
      if (existingCommand) {
        if (existingCommand.status === 'success' && existingCommand.result) {
          return Response.json({ ...existingCommand.result, idempotent_replay: true });
        }
        return Response.json({
          error: 'campaign_request_already_recorded',
          status: existingCommand.status || 'pending',
          campaign_id: existingCommand.target_id || null,
        }, { status: 409 });
      }

      const now = new Date().toISOString();
      const command = await base44.asServiceRole.entities.CommandLog.create({
        command_id: clientRequestId,
        command_type: 'notification_campaign_create_and_send',
        command_source: 'customer_app_admin',
        status: 'pending',
        target_entity: 'NotificationCampaign',
        target_id: null,
        actor_email: user.email,
        actor_role: user.role,
        actor_type: 'user',
        payload: { audience, notification_type: notificationType, deep_link_present: Boolean(deepLink) },
        idempotency_key: idempotencyKey,
        request_id: clientRequestId,
        submitted_at: now,
        function_name: 'sendNotificationCampaign',
      });
      commandLogId = command?.id || null;

      campaign = await base44.asServiceRole.entities.NotificationCampaign.create({
        title,
        message,
        audience,
      ...
      if (!campaign_id) {
        await updateCommand(base44, commandLogId, {
          status: 'failed',
          error_code: 'campaign_create_missing_id',
          error_message: 'Campaign creation did not return an id.',
          completed_at: new Date().toISOString(),
        });
        return Response.json({ error: 'campaign_create_missing_id' }, { status: 500 });
      }
      await updateCommand(base44, commandLogId, {
        status: 'running',
        target_id: campaign_id,
        target_display_id: campaign_id,
    ...
    if (requiresCampaignPreference) {
      ...
      if (normalizeSingleLine(broad_send_confirmation) !== expectedConfirmation) {
        await updateCommand(base44, commandLogId, { status: 'rejected', error_code: 'broad_campaign_confirmation_required', completed_at: new Date().toISOString() });
        return Response.json({
          success: false,
      ...
      if (!acknowledgedMax || acknowledgedMax < uniqueEmails.length) {
        await updateCommand(base44, commandLogId, { status: 'rejected', error_code: 'broad_campaign_recipient_ack_required', completed_at: new Date().toISOString() });
        return Response.json({
          success: false,
    ...
    await updateCommand(base44, commandLogId, {
      ...
      ...(finalStatus === 'sent' ? {} : { error_code: 'campaign_delivery_failed' }),
      completed_at: new Date().toISOString(),
    });
