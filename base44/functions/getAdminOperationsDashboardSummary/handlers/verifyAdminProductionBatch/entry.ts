...
function sanitizeHubCommandResponse(data, requestId) {
  return {
    ...
    verified_at: sanitizeText(data?.verified_at, 80) || null,
    verified_by: sanitizeText(data?.verified_by, 160) || null,
    compliance_log_id: sanitizeText(data?.compliance_log_id, 180) || null,
    request_id: sanitizeText(data?.request_id, 180) || requestId || null,
    skipped: data?.skipped === true,
    updated_at: sanitizeText(data?.updated_at, 80) || null,
    batch_locked: data?.batch_locked === true,
...
function safeHubError(data, fallback = 'Unable to verify Hub production batch') {
...
export default async function handler(req: Request) {
  try {
    ...
    if (!HUB_API_URL || !CUSTOMER_APP_SYNC_SECRET) {
      return Response.json({ error: 'Hub production batch verify command service is not configured' }, { status: 503 });
    }

    const hubBase = HUB_API_URL.replace(/\/$/, '').replace(/\/api\/functions\/.*$/, '').replace(/\/functions\/.*$/, '');
    const hubResponse = await fetch(`${hubBase}/functions/verifyProductionBatchForCustomerApp`, {
      method: 'POST',
      headers: {
    ...
    console.error('[verifyAdminProductionBatch] Error');
    return Response.json({ error: 'Unable to verify Hub production batch' }, { status: 500 });
  }
}
