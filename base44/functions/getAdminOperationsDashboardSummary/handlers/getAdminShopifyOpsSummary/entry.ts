...
function effectiveShopifyOpsStatus(order, customerOrder = null) {
  const statuses = [
    customerOrder?.status,
    customerOrder?.fulfillment_status,
    order?.fulfillment_status,
    order?.shopify_fulfillment_status,
    order?.production_status,
  ].map(normalizeLower).filter(Boolean);

  if (statuses.some(status => ['refunded', 'refund_complete'].includes(status))) return 'refunded';
  if (statuses.some(status => ['canceled', 'cancelled'].includes(status))) return 'canceled';
  if (statuses.some(status => ['delivered', 'picked_up', 'fulfilled', 'completed', 'complete'].includes(status)) || customerOrder?.delivered_at) {
    return 'fulfilled';
  }
  return normalizeLower(customerOrder?.status || order?.production_status || order?.fulfillment_status || 'new').replace(/\s+/g, '_');
}

function sanitizeOrder(order, customerOrder = null) {
  const effectiveStatus = effectiveShopifyOpsStatus(order, customerOrder);
  return {
    ...
    customer_notes: safeString(order?.customer_notes, 400),
    internal_notes: safeString(order?.internal_notes, 400),
    ...
    production_status: safeString(effectiveStatus, 80),
    raw_production_status: safeString(order?.production_status, 80),
    status_source: effectiveStatus !== normalizeLower(order?.production_status) ? 'customer_app_order' : 'shopify_order_mirror',
...
function sanitizeProduct(product) {
  return {
    ...
    variants: Array.isArray(product?.variants) ? product.variants.slice(0, 8).map(variant => ({
...
function sanitizeNativeProduct(product) {
  const price = product?.price === null || product?.price === undefined ? null : safeNumber(product.price);
  return {
    id: safeString(product?.id, 140),
    shopify_product_id: safeString(product?.shopify_product_id, 140),
    title: safeString(product?.title, 180),
    handle: safeString(product?.handle || product?.slug, 180),
    product_type: safeString(product?.category || product?.product_type || 'Customer App catalog', 120),
    status: product?.is_available === false ? 'unavailable' : 'active',
    vendor: 'NuVira Juice Co.',
    tags: safeStringArray(product?.tags),
    image_url: safeString(product?.image_url, 500),
    variants: [{
      shopify_variant_id: safeString(product?.shopify_variant_id, 140),
      title: safeString(product?.size || 'Default', 160),
      sku: safeString(product?.sku, 80),
      price,
      compare_at_price: null,
      inventory_quantity: null,
      inventory_policy: null,
    }],
    synced_at: safeString(product?.updated_date, 80),
    created_date: safeString(product?.created_date, 80),
  };
}
...
export default async function handler(req: Request) {
  try {
    ...
    const [ordersRaw, customerOrdersRaw, alertsRaw, productsRaw, nativeProductsRaw, syncLogsRaw, webhookLogsRaw] = await Promise.all([
      ...
      base44.asServiceRole.entities.Product.list('sort_order', 100).catch(error => {
        console.warn('[getAdminShopifyOpsSummary] Product catalog unavailable:', error.message);
        return [];
      }),
    ...
    const orders = ordersRaw.slice(0, 20).map(order => {
      const customerOrder = (
    ...
    const alerts = alertsRaw.slice(0, 20).map(sanitizeAlert);
    const productSourceRows = productsRaw.length > 0 ? productsRaw : nativeProductsRaw;
    const productSource = productsRaw.length > 0 ? 'shopify_product_mirror' : 'customer_app_product_catalog';
    const products = productSourceRows.slice(0, 20).map(productsRaw.length > 0 ? sanitizeProduct : sanitizeNativeProduct);
    const sync_logs = syncLogsRaw.slice(0, 10).map(sanitizeSyncLog);
    const webhook_logs = webhookLogsRaw.slice(0, 10).map(sanitizeWebhookLog);
    ...
    return Response.json({
      ...
      summary: {
        ...
        alert_count: alertsRaw.length,
        alerts_returned: alerts.length,
        unread_alert_count: alertsRaw.filter(alert => alert.is_read !== true).length,
        product_count: productSourceRows.length,
        products_returned: products.length,
        sync_log_count: syncLogsRaw.length,
        sync_logs_returned: sync_logs.length,
        webhook_log_count: webhookLogsRaw.length,
        webhook_logs_returned: webhook_logs.length,
      },
      response_windowed: orders.length < ordersRaw.length || products.length < productsRaw.length || alerts.length < alertsRaw.length,
