...
async function loadNativeDeliveryStops(base44, deliveryDate, limit, testTaskMode = 'exclude') {
  ...
  async function readList(entity, sort) {
    if (!entity || typeof entity.list !== 'function') {
      return { available: false, rows: [] };
    }
    try {
      const rows = await entity.list(sort, 500);
  ...
  const [taskRead, orderRead, customerOrderRead, profileRead] = await Promise.all([
    readList(base44.asServiceRole.entities.FulfillmentTask, '-delivery_date'),
    includeOrderSources
      ? readList(base44.asServiceRole.entities.ShopifyOrder, '-created_date')
      : Promise.resolve({ available: true, rows: [] }),
    includeOrderSources
      ? readList(base44.asServiceRole.entities.Order, '-created_date')
      : Promise.resolve({ available: true, rows: [] }),
    includeOrderSources
      ? readList(base44.asServiceRole.entities.UserProfile, '-updated_date')
      : Promise.resolve({ available: true, rows: [] }),
  ]);
  const tasks = taskRead.rows;
  const orders = orderRead.rows;
  const customerOrders = customerOrderRead.rows;
  const userProfiles = profileRead.rows;
  const requiredReads = includeOrderSources
    ? [taskRead, orderRead, customerOrderRead]
    : [taskRead];
  const sourceAvailable = requiredReads.some(read => read.available === true);
  const readFailureCount = requiredReads.filter(read => read.available !== true).length;
  ...
  return {
    ...
    native_read_failure_count: readFailureCount,
  };
}
