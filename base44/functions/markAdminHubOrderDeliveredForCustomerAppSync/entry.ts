...
function findForbiddenBodyKey(body) {
  ...
  for (const key of Object.keys(body)) {
    ...
    if (/(^|_)(customer|order|task|batch|inventory|review_queue|delivery|route|proof|provider|stripe|shopify|payment|notification|sync)_(id|ids|status|update|mutation|payload|name|email|phone|address|fields|url|file|flag|flags)$/i.test(normalized)) {
      return key;
    }
...
function sanitizeHubCommandResponse(data, requestId) {
  return {
    ...
    hub_order_id: sanitizeText(data?.hub_order_id, 160) || null,
    fulfillment_task_id: sanitizeText(data?.fulfillment_task_id, 160) || null,
    previous_production_status: sanitizeText(data?.previous_production_status, 40) || null,
    production_status: sanitizeText(data?.production_status, 40) || null,
    previous_fulfillment_status: sanitizeText(data?.previous_fulfillment_status, 40) || null,
    fulfillment_status: sanitizeText(data?.fulfillment_status, 40) || null,
    ...
    sync_required: data?.sync_required === true,
    notification_expected_after_sync: data?.notification_expected_after_sync === true,
    proof_drop_omitted: data?.proof_drop_omitted === true,
    fake_test_only: data?.fake_test_only === true,
  };
}

function safeHubError(data, fallback = 'Unable to mark Hub order delivered for Customer App sync') {
...
Deno.serve(async (req) => {
  try {
    ...
    try {
      ...
      hubOrderId = normalizeId(body.hub_order_id, 'hub_order_id', false);
    ...
    if (!HUB_API_URL || !CUSTOMER_APP_SYNC_SECRET) {
      return Response.json({ error: 'Hub customer delivered sync command service is not configured' }, { status: 503 });
    }
    ...
    if (hubOrderId) {
      hubBody.hub_order_id = hubOrderId;
    }
    ...
    const hubResponse = await fetch(`${hubBase}/functions/markHubOrderDeliveredForCustomerAppSync`, {
      method: 'POST',
      headers: {
    ...
    console.error('[markAdminHubOrderDeliveredForCustomerAppSync] Error');
