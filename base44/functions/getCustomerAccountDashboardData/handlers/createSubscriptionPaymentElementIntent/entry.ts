    ...
    try {
      ...
      if (existingStripeCustomer) {
        ...
        if (activeStripeSubs.data.length > 0) {
          ...
          // The customer checkout must never call a retired admin repair endpoint.
          // Stripe remains authoritative here; an admin reconciliation can be run
          // separately after the customer is safely blocked from duplicating the plan.
          return Response.json({
