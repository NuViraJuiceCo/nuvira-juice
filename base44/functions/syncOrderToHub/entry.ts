...
Deno.serve(async (req) => {
  try {
    ...
    console.log('syncOrderToHub triggered. Payload keys:', Object.keys(body));

    // Entity automation payload: { event, data, old_data }
    const order = body.data;

    if (!order || !order.id) {
      console.error('No order data in payload:', JSON.stringify(body).substring(0, 500));
    ...
    const notificationMessage = itemsCount > 2 ? `${itemsPreview} + ${itemsCount - 2} more` : itemsPreview;
    ...
    console.error('syncOrderToHub error:', error.message, error.stack);
