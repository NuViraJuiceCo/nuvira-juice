...
function slugForBatch(value) {
  const key = normalizeMatchKey(value).replace(/\s+/g, '-').toUpperCase();
  return key || 'PRODUCT';
}
...
function sourceTextForBatch(batch) {
  return `${JSON.stringify(batch?.order_sources || [])} ${JSON.stringify(batch?.related_orders || [])}`;
}

function batchStatusBlocksDemandUpdate(batch) {
...
function batchSameProductAndDate(batch, productName, productionDate) {
  return normalizeMatchKey(batch?.product_name) === normalizeMatchKey(productName) && normalizeText(batch?.production_date) === normalizeText(productionDate);
}

function batchContainsOrderSource(batch, identifiers) {
  const text = sourceTextForBatch(batch);
  return (identifiers || []).filter(Boolean).some(identifier => text.includes(String(identifier)));
}

function safeExistingBatchRows(existingBatches, readiness, identifiers) {
  const productionDate = readiness.production_date;
  const demandProducts = new Set((readiness.production_demand_rows || []).map(row => normalizeMatchKey(row.product_name)).filter(Boolean));
  return (existingBatches || [])
    .filter(batch => {
      const sameDate = !productionDate || normalizeText(batch?.production_date) === productionDate;
      const sameDemandProduct = demandProducts.has(normalizeMatchKey(batch?.product_name));
      const sourceMatch = batchContainsOrderSource(batch, identifiers);
      return sameDate && (sameDemandProduct || sourceMatch);
    })
    .slice(0, DEFAULT_MAX_ROWS)
    .map(batch => ({
      production_batch_id: sanitizeId(batch?.id, 120),
      ...
      same_product_date_match: demandProducts.has(normalizeMatchKey(batch?.product_name)) && normalizeText(batch?.production_date) === productionDate,
      source_system: sanitizeText(batch?.source_system, 80),
      native_owner_status: sanitizeText(batch?.native_owner_status, 80),
    }));
}

function buildOrderSourceRows(readiness) {
  const orderNumber = readiness.order_number;
  return (readiness.production_demand_rows || []).map(row => ({
    order_number: orderNumber,
    native_shopify_order_id: readiness.native_shopify_order_id || null,
    base44_order_id: readiness.customer_app_order_id || null,
    native_fulfillment_task_id: readiness.native_fulfillment_task_id || null,
    product_name: sanitizeText(row.product_name, 120),
    quantity_contribution: roundQuantity(row.quantity, 3),
    source_type: 'customer_app_native_order',
    source_line_item: sanitizeText(row.source_line_item, 120),
    demand_source_type: sanitizeText(row.demand_source_type, 80),
    parent_bundle: sanitizeText(row.bundle_name, 120),
    bundle_component: sanitizeText(row.bundle_component, 120),
    production_date: readiness.production_date || null,
    delivery_date: readiness.delivery_date || null,
  })).filter(row => row.product_name && row.quantity_contribution > 0);
}

function buildProposedBatchRows({ readiness, existingBatches, identifiers }) {
  const sourceRows = buildOrderSourceRows(readiness);
  const grouped = new Map();
  for (const row of sourceRows) {
    const key = `${readiness.production_date || 'date-pending'}::${normalizeMatchKey(row.product_name)}`;
    ...
    current.planned_units += safeNumber(row.quantity_contribution, 0);
    if (row.order_number) current.source_order_numbers.add(row.order_number);
    current.source_rows.push(row);
    grouped.set(key, current);
  }

  return [...grouped.values()].map(group => {
    const sameProductDateBatches = (existingBatches || [])
      .filter(batch => batchSameProductAndDate(batch, group.product_name, group.production_date));
    const exactSourceBatch = sameProductDateBatches.find(batch => batchContainsOrderSource(batch, identifiers));
    const existingBatch = exactSourceBatch || sameProductDateBatches[0] || null;
    const existingConflict = existingBatch && !exactSourceBatch && batchStatusBlocksDemandUpdate(existingBatch);
    const rowWarnings = [];
    const rowBlockers = [];
    if (exactSourceBatch) rowWarnings.push('existing_native_batch_already_contains_order_source');
    if (existingBatch && !exactSourceBatch && !existingConflict) rowWarnings.push('existing_native_batch_same_product_date_would_need_order_source_merge');
    if (existingConflict) rowBlockers.push(`existing_conflicting_native_batch:${sanitizeText(existingBatch.batch_id || existingBatch.id || group.product_name, 120)}`);

    const batchKey = `BATCH-${group.production_date || 'DATE-PENDING'}-${slugForBatch(group.product_name)}`;
    return {
      batch_key: batchKey,
      batch_id: batchKey,
      production_date: group.production_date,
      product_name: sanitizeText(group.product_name, 120),
      product_category: productCategoryForBatch(group.product_name),
      planned_units: roundQuantity(group.planned_units, 3),
      source_order_count: group.source_order_numbers.size,
      source_order_numbers: [...group.source_order_numbers].sort(),
      proposed_status: 'planned',
      would_create: !existingBatch,
      would_update_existing: Boolean(existingBatch && !exactSourceBatch && !existingConflict),
...
function buildMaterializationPreview({ readiness, existingBatches }) {
  const identifiers = [
    readiness.order_number,
    readiness.native_shopify_order_id,
    readiness.customer_app_order_id,
    readiness.native_fulfillment_task_id,
  ].filter(Boolean);
  const materializationBlockers = [];
  const warnings = [...(readiness.warnings || [])];

  if (!readiness.customer_app_order_present) materializationBlockers.push('order_not_found');
  if (readiness.payment_status !== 'paid') materializationBlockers.push('order_not_paid');
  if (readiness.payment_captured !== true) materializationBlockers.push('payment_not_captured');
  if (!readiness.native_shopify_order_present) materializationBlockers.push('missing_native_shopify_order');
  if (readiness.fulfillment_type === 'delivery' && !readiness.native_fulfillment_task_present) materializationBlockers.push('missing_native_fulfillment_task');
  if (!readiness.production_date) materializationBlockers.push('missing_production_date');
  if (!readiness.line_item_count) materializationBlockers.push('missing_line_items');
  if (!readiness.production_ready) materializationBlockers.push('production_demand_not_ready');
  for (const blocker of readiness.production_blockers || []) {
    if (!materializationBlockers.includes(blocker)) materializationBlockers.push(blocker);
  }

  const proposedOrderSourceRows = buildOrderSourceRows(readiness);
  const proposedBatchRows = buildProposedBatchRows({ readiness, existingBatches, identifiers });
  if (proposedBatchRows.length === 0) materializationBlockers.push('no_proposed_production_batch_rows');
  for (const row of proposedBatchRows) {
    for (const blocker of row.blockers || []) materializationBlockers.push(blocker);
    for (const warning of row.warnings || []) warnings.push(warning);
  }

  const existingNativeBatchMatches = safeExistingBatchRows(existingBatches, readiness, identifiers);
  const uniqueBlockers = [...new Set(materializationBlockers)].slice(0, MAX_BLOCKERS);
  const uniqueWarnings = [...new Set([
    ...warnings,
    readiness.procurement_needed ? 'procurement_needed' : null,
    readiness.procurement_conversion_ready === false ? 'procurement_conversion_pending' : null,
    'inventory_deduction_held',
    'purchase_order_automation_held',
    'hub_fallback_required',
  ].filter(Boolean))].slice(0, MAX_BLOCKERS);
  const materializationReady = uniqueBlockers.length === 0;

  return {
    success: true,
    dry_run: true,
    writes_performed: false,
    function_name: 'previewNativeProductionDemandMaterialization',
    order_number: readiness.order_number || null,
    customer_app_order_present: readiness.customer_app_order_present,
    native_shopify_order_present: readiness.native_shopify_order_present,
    native_fulfillment_task_present: readiness.native_fulfillment_task_present,
    customer_app_order_id: readiness.customer_app_order_id || null,
    native_shopify_order_id: readiness.native_shopify_order_id || null,
    native_fulfillment_task_id: readiness.native_fulfillment_task_id || null,
    payment_status: readiness.payment_status || null,
    payment_captured: readiness.payment_captured === true,
    production_ready: readiness.production_ready === true,
    production_date: readiness.production_date || null,
    delivery_date: readiness.delivery_date || null,
    fulfillment_type: readiness.fulfillment_type || null,
    order_type: readiness.order_type || null,
    line_item_count: readiness.line_item_count || 0,
    product_demand_rows: readiness.production_demand_rows || [],
    production_demand_rows: readiness.production_demand_rows || [],
    bundle_decomposition_rows: readiness.bundle_decomposition_rows || [],
    recipe_match_rows: readiness.recipe_match_rows || [],
    ingredient_need_rows: readiness.ingredient_need_rows || [],
    proposed_production_batch_rows: proposedBatchRows,
    proposed_order_source_rows: proposedOrderSourceRows,
    existing_native_batch_matches: existingNativeBatchMatches,
    existing_hub_batch_context: {
      available: false,
      reason: 'hub_batch_context_not_loaded_in_customer_app_read_only_preview',
    },
    materialization_ready: materializationReady,
    materialization_blockers: uniqueBlockers,
    blockers: uniqueBlockers,
    warnings: uniqueWarnings,
    hub_fallback_required: true,
    procurement_needed: readiness.procurement_needed === true,
    procurement_needed_count: readiness.procurement_needed_count || 0,
    procurement_conversion_ready: readiness.procurement_conversion_ready === true,
    inventory_deduction_ready: false,
    purchase_order_ready: false,
    yield_details_pending: readiness.yield_details_pending === true,
    pending_yield_items: readiness.pending_yield_items || [],
    deferred_stock_unit_items: readiness.deferred_stock_unit_items || [],
    unsupported_stock_unit_items: readiness.unsupported_stock_unit_items || [],
    missing_recipe_items: readiness.missing_recipe_items || [],
    missing_bundle_items: readiness.missing_bundle_items || [],
    missing_inventory_items: readiness.missing_inventory_items || [],
    missing_yield_items: readiness.missing_yield_items || [],
    next_action: materializationReady
      ? (proposedBatchRows.every(row => row.would_skip_existing) ? 'native_production_demand_already_materialized_or_deduped' : 'ready_for_gated_native_production_batch_materialization_planning')
      : 'hold_materialization_blockers',
    safety: {
      dry_run_only: true,
      writes_performed: false,
      production_batches_created: false,
...
Deno.serve(async (req) => {
  try {
    ...
    const materialization = buildMaterializationPreview({ readiness, existingBatches });
    ...
    return Response.json({
      ...
      master_data_summary: readiness.master_data_summary || {
    ...
    console.error(`[previewNativeProductionDemandMaterialization] failed safely: ${error?.message || 'unknown error'}`);
    return Response.json({
      success: false,
      dry_run: true,
      error_code: 'native_production_demand_materialization_preview_failed',
      message: 'Native production demand materialization preview failed safely.',
      writes_performed: false,
    }, { status: 500 });
  }
});
