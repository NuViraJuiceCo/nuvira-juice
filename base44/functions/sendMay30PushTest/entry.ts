...
async function findPushSubscriptionRows(base44: Base44Client, email: string): Promise<PushSubscriptionRecord[]> {
  ...
  try {
    ...
    console.warn('[sendMay30PushTest] PushSubscription schema unavailable; checking fallback storage');
  }
  ...
  try {
    ...
    console.warn(`[sendMay30PushTest] Fallback subscription lookup skipped: ${errorMessage(error)}`);
  }
