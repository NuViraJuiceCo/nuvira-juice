...
Deno.serve(async (req) => {
  try {
    ...
    if (!HUB_API_URL || !CUSTOMER_APP_SYNC_SECRET) {
      return Response.json({ error: 'Hub delivered command service is not configured' }, { status: 503 });
    }
    ...
    const hubResponse = await fetch(`${hubBase}/functions/recordFulfillmentTaskDeliveredForCustomerApp`, {
      method: 'POST',
      headers: {
